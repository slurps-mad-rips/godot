...
def extract(regex, content):
    return filter(None, regex.match(content).group(1).split('\n'))

sections = dict(
    DONORS_SPONSOR_PLAT=regex('Platinum sponsors'),
    DONORS_SPONSOR_GOLD=regex('Gold sponsors'),
    DONORS_SPONSOR_MINI=regex('Mini sponsors'),
    DONORS_GOLD=regex('Gold donors'),
    DONORS_SILVER=regex('Silver donors'),
    DONORS_BRONZE=regex('Bronze donors'))
